 ...
 * Client) and the block-editor script that calls it over REST from JavaScript.
 *
 ...
 *
 * This branch calls the ability over its auto-generated REST endpoint with
 * `@wordpress/api-fetch`, so the bundle is a plain classic script — no script
 * modules involved. `@wordpress/scripts` records the WP package dependencies
 * (including `wp-api-fetch`) in `index.asset.php`; passing them straight to
 * `wp_enqueue_script()` makes the editor load them first and lets `apiFetch`
 * pick up its REST nonce automatically.
 */
